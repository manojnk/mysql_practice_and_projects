-- I Asked This SQL Interview Question in an Amazon Interview | Most Asked SQL Problem with a Twist
-- Write a SQL Query to find the employee with 3rd heighest salary in each department
-- In case there are less then 3 employees in a department then return the employee details with least salary
CREATE TABLE emp(
 emp_id int NULL,
 emp_name varchar(50) NULL,
 salary int NULL,
 manager_id int NULL,
 emp_age int NULL,
 dep_id int NULL,
 dep_name varchar(20) NULL,
 gender varchar(10) NULL
) ;
insert into emp values(1,'Ankit',14300,4,39,100,'Analytics','Female');
insert into emp values(2,'Mohit',14000,5,48,200,'IT','Male');
insert into emp values(3,'Vikas',12100,4,37,100,'Analytics','Female');
insert into emp values(4,'Rohit',7260,2,16,100,'Analytics','Female');
insert into emp values(5,'Mudit',15000,6,55,200,'IT','Male');
insert into emp values(6,'Agam',15600,2,14,200,'IT','Male');
insert into emp values(7,'Sanjay',12000,2,13,200,'IT','Male');
insert into emp values(8,'Ashish',7200,2,12,200,'IT','Male');
insert into emp values(9,'Mukesh',7000,6,51,300,'HR','Male');
insert into emp values(10,'Rakesh',8000,6,50,300,'HR','Male');
insert into emp values(11,'Akhil',4000,1,31,500,'Ops','Male');

select * from emp;

-- output table
-- emp_id, emp_name, salary, manager_id, emp_age, dep_id, dep_name, 	gender
-- 4		Rohit		7260	2			16		100		Analytics	Female
-- 2		Mohit		14000	5			48		200		IT			Male
-- 9		Mukesh		7000	6			51		300		HR			Male
-- 11		Akhil		4000	1			31		500		Ops			Male
-- ===============================================================================================================================================
-- solution 1
with cte as 
(select *
,rank() over(partition by dep_id order by salary desc) as high_salary
,rank() over(partition by dep_id order by salary asc) as low_salary
,count(emp_id) over(partition by dep_id) as no_of_employee
from emp)
select emp_id, emp_name, salary, manager_id, emp_age, dep_id, dep_name, gender
from cte 
where (no_of_employee>=3 and high_salary =3) or (no_of_employee<3 and low_salary =1);

-- solution 2
with cte as 
(select *
,rank() over(partition by dep_id order by salary desc) as rank_salary
,count(emp_id) over(partition by dep_id) as no_of_employee
from emp)
select emp_id, emp_name, salary, manager_id, emp_age, dep_id, dep_name, gender
from cte 
where (rank_salary =3) or (no_of_employee<3 and rank_salary = no_of_employee)


